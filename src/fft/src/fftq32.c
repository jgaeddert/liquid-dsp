...
//
// FFT API: fixed-point, 32-bit precision
//

#include "liquid.internal.h"

// Macro definitions
#define FFT(name)           LIQUID_CONCAT(fftq32,name)
#define DOTPROD(name)       LIQUID_CONCAT(dotprod_cccq32,name)

// fixed-point macros
#define LIQUID_FPM          1
#define Q(name)             LIQUID_CONCAT(q32,name)
#define CQ(name)            LIQUID_CONCAT(cq32,name)

#define T                   q32_t           /* primitive type */
#define TC                  cq32_t          /* primitive type (complex) */
