...
decim decim_create_generic(unsigned int _D, float _fc, float _t, float _slsl)
{
    ...
    // create buffer
    d->num_buffers = 1;
    d->buffers = (fwindow*) malloc(1*sizeof(fwindow));
    d->buffers[0] = fwindow_create(d->h_len);
...
decim decim_create_halfband(float _fc, float _t, float _slsl)
{
    ...
    // create buffers
    d->num_buffers = 2;
    d->buffers = (fwindow*) malloc(2*sizeof(fwindow));
    d->buffers[0] = fwindow_create(d->h_len);
    d->buffers[1] = fwindow_create(d->h_len);
    // TODO: fill buffer with zeros
