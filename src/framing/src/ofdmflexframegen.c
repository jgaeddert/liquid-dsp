                               ...
                               unsigned char * _payload)
{
    ...
    memmove(_q->header, _header, 12*sizeof(unsigned char));
...
{
    // first 12 bytes user data

    // first byte is for expansion/version validation
    _q->header[12] = OFDMFLEXFRAME_VERSION;

    // add payload length
    unsigned int payload_len = _q->props.payload_len;
    _q->header[14] = (payload_len >> 8) & 0xff;
    _q->header[15] = (payload_len     ) & 0xff;
    ...
    _q->header[16]  = ( _q->props.mod_scheme & 0x1f) << 3;
    _q->header[16] |= ((_q->props.mod_bps-1) & 0x07);
    ...
    _q->header[17]  = (_q->props.check & 0x07) << 5;
    _q->header[17] |= (_q->props.fec0) & 0x1f;
    _q->header[18]  = (_q->props.fec1) & 0x1f;
...
void ofdmflexframegen_modulate_header(ofdmflexframegen _q)
{
    ...
    for (i=0; i<32; i++) {
        _q->header_mod[8*i+0] = (_q->header_enc[i] >> 7) & 0x01;
        _q->header_mod[8*i+1] = (_q->header_enc[i] >> 6) & 0x01;
        _q->header_mod[8*i+2] = (_q->header_enc[i] >> 5) & 0x01;
        _q->header_mod[8*i+3] = (_q->header_enc[i] >> 4) & 0x01;
        _q->header_mod[8*i+4] = (_q->header_enc[i] >> 3) & 0x01;
        _q->header_mod[8*i+5] = (_q->header_enc[i] >> 2) & 0x01;
        _q->header_mod[8*i+6] = (_q->header_enc[i] >> 1) & 0x01;
        _q->header_mod[8*i+7] = (_q->header_enc[i]     ) & 0x01;
    }
}
    ...
    for (i=0; i<_q->M; i++) {
        ...
        if (sctype == OFDMFRAME_SCTYPE_DATA) {
            ...
            if (_q->header_symbol_index < 256) {
                ...
                //printf("  writing symbol %3u / %3u (x = %8.5f + j%8.5f)\n", _q->header_symbol_index, 256, crealf(_q->X[i]), cimagf(_q->X[i]));
            } else {
