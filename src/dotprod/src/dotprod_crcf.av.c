...
void dotprod_crcf_destroy(dotprod_crcf _dp)
{
...
void dotprod_crcf_print(dotprod_crcf _dp)
{
    printf("dotprod_crcf [%u taps]:\n", _dp->n);
    unsigned int i;
    for (i=0; i<_dp->n; i++)
        printf("  %3u : %12.9f\n", i, _dp->h[0][2*i]);
}
...
void dotprod_crcf_execute(dotprod_crcf _dp,
                          float complex * _x,
                          float complex * _r)
{
