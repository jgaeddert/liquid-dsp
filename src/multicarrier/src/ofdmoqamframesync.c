...
#define DEBUG_OFDMOQAMFRAMESYNC_PILOTPHASE  0

// auto-correlation integration length
#define OFDMOQAMFRAMESYNC_AUTOCORR_LEN      (96)
                                               ...
                                               void * _userdata)
{
    ...
    ofdmoqamframe_init_S0(q->S0);
    ofdmoqamframe_init_S1(q->S1);
    ofdmoqamframe_init_S2(q->S2);
    unsigned int i;
    ...
    q->autocorr_length = OFDMOQAMFRAMESYNC_AUTOCORR_LEN;
...
void ofdmoqamframesync_execute_plcplong0(ofdmoqamframesync _q, float complex _x)
{
...
#if DEBUG_OFDMOQAMFRAMESYNC_PRINT
        printf("warning: ofdmoqamframesync could not find first PLCP long sequence; resetting synchronizer\n");
#endif
    ...
    if (cabsf(rxy) > (_q->rxy_thresh)*(_q->num_subcarriers)) {
        ...
        _q->state = OFDMOQAMFRAMESYNC_STATE_PLCPLONG1;
        _q->timer = 0;
    }
}
...
#if DEBUG_OFDMOQAMFRAMESYNC_PRINT
        printf("warning: ofdmoqamframesync could not find second PLCP long sequence; resetting synchronizer\n");
#endif
        ofdmoqamframesync_reset(_q);
        return;
    }
    ...
    if ((_q->num_samples % 64) == k) {
        ...
        if (_q->num_symbols == _q->m) {
...
#endif
            ofdmoqamframesync_estimate_gain_plcplong(_q);
#if DEBUG_OFDMOQAMFRAMESYNC
            ...
            */
            ...
            // execute rxpayload
            ofdmoqamframesync_rxpayload(_q,_q->Y0,_q->Y1);
                                   ...
                                   float complex * _Y1)
{
    ...
    for (i=0; i<_q->num_subcarriers; i++) {
        ...
        if (sctype==OFDMOQAMFRAME_SCTYPE_PILOT) {
            ...
            float phi_hat =
            ofdmoqamframesync_estimate_pilot_phase(y0,y1,p);
...
void ofdmoqamframesync_estimate_gain_plcplong(ofdmoqamframesync _q)
{
    ...
    for (j=0; j<_q->num_subcarriers; j++) {
        ...
        if (sctype==OFDMOQAMFRAME_SCTYPE_NULL) {
            ...
            int sctype0 = ofdmoqamframe_getsctype(j0);
            int sctype1 = ofdmoqamframe_getsctype(j1);
            float complex g=0.0f;
            float w=0.0f;
            if (sctype0 != OFDMOQAMFRAME_SCTYPE_NULL) {
                g += _q->G[j0];
                w += 1.0f;
            }
            if (sctype1 != OFDMOQAMFRAME_SCTYPE_NULL) {
                g += _q->G[j1];
                w += 1.0f;
            }
            _q->G[j] = g / w;
...
#if DEBUG_OFDMOQAMFRAMESYNC_PILOTPHASE
    FILE * fid = fopen("ofdmoqamframesync_estimate_pilot_phase.m","w");
    fprintf(fid,"close all;\n");
    fprintf(fid,"clear all;\n");
#endif
    while (phi <= phi1) {
