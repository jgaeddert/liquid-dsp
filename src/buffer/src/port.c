...
port port_create(unsigned int _n)
{
    ...
    p->buffer = cfbuffer_create(CIRCULAR,_n);
...
{
    // lock mutex (write)
    cfbuffer_write(_p->buffer, _v, _n);
    // unlock mutex (write)
