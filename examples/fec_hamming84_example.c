...
int main() {
    ...
    // encode message
    fec_hamming84_encode(data, n, msg_enc);
    ...
    msg_enc[0] ^= 0x08; // position 4
    msg_enc[1] ^= 0x04; // position 3
    msg_enc[2] ^= 0x02; // position 2
    msg_enc[3] ^= 0x01; // position 1
    ...
    // decode message
    num_errors_detected =
        fec_hamming84_decode(msg_enc, n, msg_dec);
