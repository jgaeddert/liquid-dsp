...
int main(int argc, char* argv[])
{
    ...
    liquid_argparse_add(char*, filename, "msresamp2_crcf_example.m", 'o', "output filename", NULL);
    liquid_argparse_add(float,        r,  0.25f, 'r', "resampling rate (output/input)", NULL);
    liquid_argparse_add(float,        As, 60.0f, 'a', "resampling filter stop-band attenuation [dB]", NULL);
    liquid_argparse_add(unsigned int, n,    128, 'n', "number of low-rate samples", NULL);
    liquid_argparse_add(float,        fc, 0.45f, 'f', "filter pass-band cut-off frequency", NULL);
