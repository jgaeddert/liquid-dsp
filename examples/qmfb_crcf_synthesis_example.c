...
int main() {
    ...
    nco_crcf_set_frequency(nco_0, 0.122*2*M_PI);
    nco_crcf_set_frequency(nco_1, 0.779*2*M_PI);
    float complex y0,y1;
    for (i=0; i<num_samples; i++) {
            // compute y0 (low channel pulse)
            nco_crcf_cexpf(nco_0, &y0);
            y[0][i] = y0 * kaiser(i,num_samples,10.0f,0.0f);
            nco_crcf_step(nco_0);

            // compute y1 (upper channel pulse)
            nco_crcf_cexpf(nco_1, &y1);
    ...
    fprintf(fid,"    plot(f/2,Y0,'LineWidth',1,'Color',[0 0.5 0]);\n");
    fprintf(fid,"    plot(f(t0)/2+0.5,Y1(t0),'LineWidth',1,'Color',[0.5 0 0]);\n");
    fprintf(fid,"    plot(f(t1)/2-0.5,Y1(t1),'LineWidth',1,'Color',[0.5 0 0]);\n");
    fprintf(fid,"hold off;\n");
    fprintf(fid,"grid on;\nxlabel('normalized frequency');\nylabel('PSD [dB]');\n");
    fprintf(fid,"legend('composite','Y_0','Y_1',1);");
