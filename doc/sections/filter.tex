...
\[
    H_a(s) = k
            \frac{
                (s-z_0)(s-z_1)\cdots(s-z_{n-1})
            }{
                (s-p_0)(s-p_1)\cdots(s-p_{m-1})
            }
\]
The result is a digital filter with complex coefficients arrays
$\vec{b}$ and $\vec{a}$ each of length $m+1$ where $m$ is the number of poles.

% cheby1 : chebyshev iir filter design
\begin{figure}
\centering
