...
\item[{\tt autocorr\_cccf\_destroy(q)}]
    ...
    memory.
\item[{\tt autocorr\_cccf\_reset(q)}]
...
% firdecim
%
\subsection{{\tt firdecim} (decimator)}
\label{module:filter:firdecim}
The {\tt firdecim} object family implements a basic interpolator with an
integer output-to-input resampling ratio $M$.
...
Figure~\ref{fig:module:filter:firdecim_crcf}.
%
\begin{figure}
\centering
  \includegraphics[trim = 0mm 0mm 0mm 0mm, clip, width=13cm]{figures.gen/filter_firdecim_crcf}
\caption{{\tt firdecim\_crcf} (decimator) example with $M=4$,
         compensating for filter delay.}
\label{fig:module:filter:firdecim_crcf}
\end{figure}
%
Listed below is the full interface to the {\tt firdecim} family of
objects.
While each method is listed for {\tt firdecim\_crcf}, the same
functionality applies to {\tt firdecim\_rrrf} and {\tt firdecim\_cccf}.
%
\begin{description}
\item[{\tt firdecim\_crcf\_create(M,*h,N)}]
    creates a {\tt firdecim} object with a decimation factor $M$ using $N$
    filter coefficients $\vec{h}$.
\item[{\tt firdecim\_crcf\_create\_prototype(M,m,As)}]
    creates a {\tt firdecim} object from a prototype filter with a
    decimation factor $M$,
    a delay of $M m$ samples, and
    a stop-band attenuation $A_s$~dB.
\item[{\tt firdecim\_crcf\_create\_rnyquist(type,M,m,beta,dt)}]
    creates a {\tt firdecim} object from a square-root Nyquist prototype filter with a
    decimation factor $M$,
    a delay of $M m$ samples,
    an excess bandwidth factor $\beta$, and
    a fractional sample delay $\Delta t$
    (see \S\ref{module:filter:firdes:rnyquist} for details).
\item[{\tt firdecim\_crcf\_destroy(q)}]
    destroys a {\tt firdecim} object, freeing all internally-allocated
    memory.
\item[{\tt firdecim\_crcf\_print(q)}]
    prints the parameters of a {\tt firdecim} object to the standard
    output.
\item[{\tt firdecim\_crcf\_clear(q)}]
    clears the internal buffer of a {\tt firdecim} object.
\item[{\tt firdecim\_crcf\_execute(q,*x,*y,k)}]
    computes the output decimation of the input sequence $\vec{x}$
    (which is $M$ samples in size) at the index $k$ and stores the
    result in $y$.
\end{description}
%
An example of the {\tt firdecim} interface is listed below.
%
\input{listings/firdecim.example.c.tex}
%
A more detailed example is given in
{\tt examples/firdecim\_crcf\_example.c}
in the main \liquid\ project directory.
...
% firinterp
%
\subsection{{\tt firinterp} (finite impulse reponse interpolator)}
\label{module:filter:firinterp}
The {\tt firinterp} object implements a basic interpolator with an integer
output-to-input resampling ratio.
An example of the {\tt firinterp} interface is listed below.
%
\input{listings/firinterp.example.c.tex}
%
Listed below is the full interface to the {\tt firinterp} family of
objects.
While each method is listed for {\tt firinterp\_crcf}, the same
functionality applies to {\tt firinterp\_rrrf} and {\tt firinterp\_cccf}.
%
\begin{description}
\item[{\tt firinterp\_crcf\_create(M,*h,N)}]
    creates an {\tt firinterp} object with an interpolation factor $M$
    using $N$ filter coefficients $\vec{h}$.
\item[{\tt firinterp\_crcf\_create\_prototype(M,m,As)}]
    create an {\tt firinterp} object using a filter prototype designed
    using the {\tt firdes\_kaiser\_window()} method
    (see \S\ref{module:filter:firdes})
    with a normalized cut-off frequency $1/2M$,
    a filter length of $2Mm$ coefficients,
    and a stop-band attenuation of $A_s$~dB.
\item[{\tt firinterp\_crcf\_create\_rnyquist(type,k,m,beta,dt)}]
    creates an {\tt firinterp} object
...
\item[{\tt firinterp\_crcf\_destroy(q)}]
    destroys the firinterpolator, freeing all internally-allocated memory.
\item[{\tt firinterp\_crcf\_print(q)}]
    prints the internal properties of the interpolator to the standard
    output.
\item[{\tt firinterp\_crcf\_clear(q)}]
    clears the internal interpolator buffers.
\item[{\tt firinterp\_crcf\_execute(q,x,*y)}]
...
\begin{figure}
\centering
  \includegraphics[trim = 0mm 0mm 0mm 0mm, clip, width=13cm]{figures.gen/filter_firinterp_crcf}
\caption{{\tt firinterp\_crcf} (interpolator) example with $M=4$,
         compensating for filter delay.}
\label{fig:module:filter:firinterp_crcf}
\end{figure}
%
A graphical example of the interpolator can be seen in
Figure~\ref{fig:module:filter:firinterp_crcf}.
A detailed example program is given in
{\tt examples/firinterp\_crcf\_example.c},
