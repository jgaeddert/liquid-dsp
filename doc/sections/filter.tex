...
All square-root Nyquist filters in \liquid\ have four basic properties:
%
\begin{description}
\item[$k$] samples per symbol
\item[$m$] filter delay (symbols)
\item[$\beta$] excess bandwidth factor
\item[$\Delta t$] fractional sample delay, usually set to zero for
    typical filter designs
\end{description}
%
The resulting filter will have $N = 2 k m + 1$ coefficients.
% ... xxx
%
\begin{figure}
\centering
