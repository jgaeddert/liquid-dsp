...
\item[{\tt window\_recreate()}]
    ...
    {\tt realloc()}.
    If the size of the new window is larger than the old one, the old values
    are pushed to the end.
\item[{\tt window\_clear()}]
    clears the contents of the buffer (set to zero)
...
The comments show the internal state of the window after each function call.
