...
values
and add channel impairments (noise and carrier frequency offset).
...
\item What happens to the spectral efficiency of the frame when you
      ...
      % answer: efficiency increases from 0.5357 to 0.8439
      % reason: preamble accounts for less of frame (less overhead)
      when you decrease the cyclic prefix from 16 samples to 4?
      % answer: efficiency increases from 0.5357 to 0.6186
      % reason: fewer overhead samples sent for each OFDM symbol
      when you increase the number of subcarriers from 64 to 256
      % answer: efficiency decreases from 0.5357 to 0.400
      % reason: preamble accounts for MORE of frame (more overhead)
\item What happens when the frame generator is created with 64
      subcarriers and the synchronizer is created with only 62?
      % answer: synchronizer cannot detect frame
      % reason: different pilots, etc.
      when the cyclic prefix lengths don't match?
      % answer: synchronizer cannot decode header
      % reason: timing mis-alignment
%\item What happens when you decrease the SNR from 30~dB to 20~dB?
