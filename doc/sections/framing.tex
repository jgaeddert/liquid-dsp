...
\item[{\tt packetizer\_decode\_soft(q,*pkt,*msg)}]
    ...
    and a {\tt 0} if it failed.
    See Section~\ref{module:fec:soft} for more information on
...
\item[{\tt evm}]
    ...
    (see Section~\ref{module:modem:digital}).
\item[{\tt rssi}]
...
\end{itemize}
%
The {\tt ofdmflexframegenprops\_init\_default(M,*p)} interface initializes
the subcarrier allocation for a system with $M$ channels
that is expected to perform relatively well under a variety of channel
conditions.
