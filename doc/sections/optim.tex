...
\begin{figure}
\centering
  \includegraphics[trim = 12mm 5mm 14mm 8mm, clip, width=13cm]{figures.gen/optim_gradsearch}
\caption{
    {\tt gradsearch} performance for Rosenbrock function
...
$f = (1-x)^2 + 100(y-x^2)^2$ for input parameters $x$ and $y$.
...
\item[{\tt ga\_search\_create(*utility,*userdata,parent,min/max)}]
    ...
    {\tt LIQUID\_OPTIM\_MINIMIZE} or
    {\tt LIQUID\_OPTIM\_MAXIMIZE}.
    %When run the {\tt ga\_search} object will update the ``optimal''
    %value in the input vector $\vec{v}$ specified during
    %{\tt create()}.
%\item[{\tt ga\_search\_create\_advanced()}]
