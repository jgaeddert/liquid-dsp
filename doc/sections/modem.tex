...
%
Let us represent the received signal as...
%
\begin{equation}
...
and $n$ is a zero-mean complex Gauss random variable with
...
$s \in \{s_0,s_1,\ldots,s_{M-1}\}$ and $E\{s s^*\}=1$.
Each point is represented by the binary string
$\{c_0,c_1,\ldots,c_{m-1}\}$.
%
Assuming perfect channel knowledge, timing, and carrier offset recovery,
the log-likelihood ratio of each bit $b_k$ is shown to be
\cite[Eq.~(8)]{LeGoff:1994}
the ratio of the two conditional {\em a posteriori} probabilities
%
\[
    \Lambda(b_k) =
        \ln \frac{
            P\left(b_k=1 | \text{observation} \right)
        }{
            P\left(b_k=0 | \text{observation} \right)
        }
\]
%
...
\label{eqn:modem:digital:soft:LLR}
    \Lambda(b_k) =
        \ln \sum_{s^+\in \{s:b_k=1\}} { \exp\left\{|r-s^+|_2^2 / 2 \sigma_n^2\right\} }
       -\ln \sum_{s^-\in \{s:b_k=0\}} { \exp\left\{|r-s^-|_2^2 / 2 \sigma_n^2\right\} }
\end{equation}
%
As shown in \cite{Qiang:2003} a sub-optimal simplified LLR expression
can be obtained by replacing...
$\ln \sum_j {e^{z_j}} \approx \max_j \ln (e^{z_j}) = \max_j z_j$
which is tight as long as the sum is dominated by its largest component.
%
\begin{equation}
...
and evaluating for $\sigma_n=0.2$ gives an approximate LLR for $b_2$
equal to -7.43.
...
To circumvent this, \liquid\ only searches over a few points (typically
four) nearest to the hard-demodulated symbol.
This can be done quickly because the hard-demodulated symbol can be
found systematically for most modulation schemes
(e.g. for {\tt LIQUID\_MODEM\_QAM} only $\log_2 M$ comparisons are
needed to make a hard decision).
If no symbols are found within this set where $s_k \in {s:b_k=...}$ then
the magnitude of $\Lambda(b_k)$ is sufficiently large
