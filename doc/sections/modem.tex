...
Consider a continuous saveform with a constant envelope but with a
time-varying phase:
\[
...
\]
The {\em instantaneous frequency} of $s(t)$ is defined to be
\[
...
This can be seen graphically in the figure below.
    ...
    \begin{tikzpicture}[x=15mm, y=5mm, >=latex, font=\footnotesize]
        ...
        \draw[very thick,liquid-color-blue]
            plot[domain=-1:8, samples=51, smooth] function {1 + x + sin(1.3*x)};
    ...
    \end{tikzpicture}
    \end{center}
We can transmit an information signal $m(t)$ by varying the angle $\theta_i(t)$
of a carrier.
Frequency modulation (FM) varies the frequency of the carrier linearly
with $m(t)$.
...
\end{eqnarray*}
%
where $f_c$ is the carrier frequency, and $k_f$ is the modulation index.
...
\]
%
In \liquid\ the {\tt freqmodem} object implements an analog FM
modulator and demodulator.
Numerical integration of $m(t)$ is performed on the modulator.
% using an 8$^{th}$-order recursive integrating filter (see section XXX)
%
