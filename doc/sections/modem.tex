...
\label{module:modem:freqmodem}
Analog frequency modulation (FM) varies the frequency of a carrier wave
in direct proportion to the analog message signal.
In contrast to phase modulation, FM has the advantage of not requiring a
coherent recevier to recover the information at the receiver.
%
Consider a continuous-time carrier with a constant envelope but with a
time-varying phase:
\[
    s(t) = A_c \cos\bigl(2\pi f_c t + \varphi(t)\bigr)
\]
Let us define the generalized angle
(also known as the {\em instantaneous phase} of the carrier)
as
\[
    \theta_i(t) = 2\pi f_c t + \varphi(t)
\]
where $\varphi(t)$ is the information-bearing phase component.
The {\em instantaneous frequency} of $s(t)$ is by definition the
derivative of phase with respect to time, viz.
\[
    f_i(t) = \frac{1}{2\pi}\frac{d\theta_i(t)}{dt} = f_c + \frac{1}{2\pi}\frac{d\varphi}{dt}
\]
%Therefore
%\[
%    \theta_i(t) = 2\pi \int\limits_{0}^{t}{f_i(\tau)d\tau}
%\]
This can be seen graphically in the figure below.
%
    \begin{center}
    \begin{tikzpicture}[x=15mm, y=5mm, >=latex, font=\footnotesize]
        \draw[->] (-1,  0) -- (9, 0) node[anchor=north] {$t$};
        \draw[->] ( 0, -1) -- (0, 9) node[anchor=west]  {$\theta_i(t)$};
        \draw[very thick,liquid-color-blue,->]
            plot[domain=-1:8, samples=201] function {1 + x + sin(1.3*x)};
        % derivative: 1 + 1.3*cos(1.3*x);
        %   evaluate at     :   t = 1.4
        %   operating point :   (1.4, 3.369)
        %   slope           :   0.679
        %   y-intercept     :   2.418
        %   at t=8          :   7.853
        %   at t=7          :   7.174
        \draw[thick, dotted, ->]
            (0, 2.418) node[anchor=east] {$\theta_0$} --
            (7, 7.174) node[anchor=north,yshift=-8pt] {$2\pi f_i t + \theta_0$};
        \draw[thick, dotted] (1.4, 0) -- (1.4, 3.369);
        %\node[circle,inner sep=1mm, liquid-color-blue, fill=liquid-color-blue!20, thick] at (1.4, 3.369) {};
        %\node[circle,inner sep=1mm] at (1.4, 3.369) {x};
    \end{tikzpicture}
    \end{center}
%
We can transmit an information signal $m(t)$ by varying the angle
$\theta_i(t)$ of a carrier.
This is in general known as {\em angle modulation} and can be divided
into two types:
{\em phase modulation} (PM)
and the much more common {\em frequency modulation} (FM).
%
%\begin{minipage}[c]{0.5\textwidth}
%    \begin{center}
%    {\bf Phase Modulation (PM)} \\
%    {\scriptsize The {\em phase} of the carrier is varied linearly with $m(t)$}
%    {\footnotesize
%    \begin{eqnarray*}
...
%\begin{minipage}[c]{0.5\textwidth}
%    \begin{center}
%    {\bf Frequency Modulation (FM)} \\
%    {\scriptsize The {\em frequency} of the carrier is varied linearly with $m(t)$}
%    {\footnotesize
%    \begin{eqnarray*}
%        f_i(t) & = & f_c + k_f m(t) \\
%        %\theta_i(t) & = & 2\pi \int\limits_{0}^{t}{f_i(\tau)d\tau} \\
%        \theta_i(t) & = & 2\pi f_c t + 2\pi k_f \int\limits_{0}^{t}{m(\tau)d\tau} \\
%        s(t)        & = & A_c \cos\left(2\pi f_c t + 2\pi k_f \int\limits_{0}^{t}{ m(\tau) d\tau } \right)
%    \end{eqnarray*}
%    } % font size
%    \end{center}
%\end{minipage}
%
Frequency modulation varies the instantaneous frequency of the carrier
linearly with the message signal.
Given an input message signal $m(t)$, the transmitted signal $s(t)$
can be derived as
%
\begin{eqnarray*}
    f_i(t) & = & f_c + k_f m(t) \\
    \theta_i(t) & = & 2\pi f_c t + 2\pi k_f \int\limits_{0}^{t}{m(\tau)d\tau} \\
    s(t)        & = & A_c\cos\left(
                            2\pi f_c t + 2\pi k_f \int\limits_{0}^{t}{ m(\tau) d\tau }
                        \right)
\end{eqnarray*}
%
where $f_c$ is the carrier frequency, and $k_f$ is the modulation factor.
The modulation factor governs the relative bandwidth of the signal.
At complex baseband the transmitted signal is simply
\[
    \tilde{s}(t) = A_c\exp\left\{
                            2\pi k_f \int\limits_{0}^{t}{ m(\tau) d\tau }
                        \right\}
\]
%
In \liquid\ the {\tt freqmod} and {\tt freqdem} objects implement
FM modulation and demodulation, respectively.

\subsubsection{{\tt freqmod}, FM modulator}
\label{module:modem:freqmodem:freqmod}
For a discrete-sampled system the modulator needs to perform numerical
integration of the message signal.
To maintain stability the modulator passes the message signal through
a DC-blocking pre-filter with a transfer function
$H_0(z) = (1 - z^{-1})/(1 - 0.9995 z^{-1})$.
%(see section XXX)
Integration of $m(t)$ is accomplished internally to the {\tt freqmod}
object with an 8$^{th}$-order recursive integrating filter with
near-linear phase response corresponding to a group delay of
$7$ samples.
%(see section XXX)
The resulting signal approximation of $\theta_i(t)$ is fed through a
complex sinusoid (complex baseband).

The interface for the {\tt freqmod} object is given below:
%
\begin{description}
\item[{\tt freqmod\_create(kf)}]
    creates and returns an FM modulator {\tt freqmod} object
    with a modulation factor $k_f$.
\item[{\tt freqmod\_destroy(q)}]
    destroys an {\tt freqmod} object, freeing all internally-allocated
    memory.
\item[{\tt freqmod\_reset(q)}]
    resets the state of the {\tt freqmod} object.
\item[{\tt freqmod\_print(q)}]
    prints the internal state of the {\tt freqmod} object.
\item[{\tt freqmod\_modulate(q,m,*s)}]
    modulates the input message sample $m$ storing the output to $s$.
\end{description}

\subsubsection{{\tt freqdem}, FM demodulator}
\label{module:modem:freqmodem:freqdem}
The demodulator first passes the received signal through a non-recursive
pre-filter which helps reduce high-frequency noise to improve fidelity
in the estimation of the instantaneous frequency $f_i(t)$.
From this point there are two options for estimating $f_i(t)$:
...
%\]
%
The choice of demodulation method is provided upon creation of the
{\tt freqdem} object.
The original message signal $m(t)$ is extracted by scaling the resulting
instantaneous frequency $f_i(t)$ by $1/2\pi k_f$.
To eliminate any DC offset as a consequence of carrier frequency offset
in the received signal, the message signal is passed through a
recursive post-filter with a frequency response
$H_0(z) = (1 - z^{-1})/(1 - 0.9999 z^{-1})$.

The interface for the {\tt freqmod} object is given below:
\begin{description}
\item[{\tt freqdem\_create(kf,type)}]
    creates and returns an FM demodulator {\tt freqdem} object
    with a modulation factor $k_f$, and
    a demodulation type defined by {\tt type}.
    The demodulation type can either be
    {\tt LIQUID\_FREQDEM\_PLL} which uses a phased-locked loop or
    {\tt LIQUID\_FREQDEM\_DELAYCONJ} which uses the delay conjugate
    method.
\item[{\tt freqdem\_destroy(q)}]
    destroys an {\tt freqdem} object, freeing all internally-allocated
    memory.
\item[{\tt freqdem\_reset(q)}]
    resets the state of the {\tt freqdem} object.
\item[{\tt freqdem\_print(q)}]
    prints the internal state of the {\tt freqdem} object.
\item[{\tt freqdem\_demodulate(q,r,*y)}]
    demodulates the received input sample $r$
    storing the output to $y$.
\end{description}

% FM example
\subsubsection{FM modulation/demodulation example}
\label{module:modem:freqmodem:example}
\begin{figure}
\centering
...
\caption{FM modulation and demodulation demonstration of an audio signal with
         a modulation factor of $k=0.05$ at complex baseband.
         Demodulation uses the delay-conjugate method.}
\label{fig:modem:freqmodem}
\end{figure}
%
An example of FM modulation and demodulation using the
{\tt freqmod} and {\tt freqdem} objects can be found in
Figure~\ref{fig:modem:freqmodem}.
%
An example of the interfaces is listed below.
